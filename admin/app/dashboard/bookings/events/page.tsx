...
export default function EventBookingsPage() {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        ...
        // Filter for event bookings only (eventId is not null)
        const response = await BookingsAPI.listBookings(params);
        
        // Client-side filtering for event bookings and search
        let filteredData = response.data.filter((booking) => booking.eventId !== null && booking.eventId !== undefined);
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: false,
      render: (_: any, row: Booking) => (
        <span className="text-sm text-gray-900">{row.event?.name || 'N/A'}</span>
      ),
    },
    {
      key: 'amount',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage event bookings</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-2">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by booking number, user, event..."
                value={search}
                onChange={(e) => {
