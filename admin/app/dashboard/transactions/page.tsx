  ...
  const columns = [
    {
      ...
      render: (_: any, row: Transaction) => (
        <div>
          <Link href={`/dashboard/payments/transactions/${row.id}`} className="text-sm font-medium text-[#0e1a30] hover:underline">
    ...
    },
    {
      ...
      sortable: false,
      render: (_: any, row: Transaction) => (
        <span className="text-sm text-gray-900">{row.type?.replace(/_/g, ' ') || '-'}</span>
      ),
    },
    {
      key: 'user',
      ...
      render: (_: any, row: Transaction) => (
        <div>
          {row.user ? (
            <Link href={`/dashboard/users/${row.user.id}`} className="text-sm text-[#0e1a30] hover:underline">
              {row.user.fullName || row.user.email || 'N/A'}
            </Link>
          ) : (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage financial transactions</p>
        </div>
      ...
      <DataTable
        ...
        data={transactions}
        loading={loading}
        onRowClick={(row) => router.push(`/dashboard/payments/transactions/${row.id}`)}
